...
unsafe extern "C" fn zelda_game_specialsstart(agent: &mut L2CAgentBase) {
    frame(agent.lua_state_agent, 13.0);
	if macros::is_excute(agent) {
        if WorkModule::get_int(agent.module_accessor, FIGHTER_ZELDA_INSTANCE_WORK_ID_INT_SPECIAL_S_COOLDOWN) == 0 {
            WorkModule::set_int(agent.module_accessor, 400, FIGHTER_ZELDA_INSTANCE_WORK_ID_INT_SPECIAL_S_COOLDOWN);
            WorkModule::set_int(agent.module_accessor, DEIN_TYPE_SPEAD, FIGHTER_ZELDA_INSTANCE_WORK_ID_INT_DEIN_TYPE);
...
unsafe extern "C" fn zelda_game_specialsend(agent: &mut L2CAgentBase) {
}

unsafe extern "C" fn zelda_specials_main(fighter: &mut L2CFighterCommon) -> L2CValue {
	...
	fighter.sub_shift_status_main(L2CValue::Ptr(zelda_specials_main_loop as *const () as _))
}

unsafe extern "C" fn zelda_specials_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
...
unsafe extern "C" fn zelda_specials_pre(fighter: &mut L2CFighterCommon) -> L2CValue {
    if !is_phosphora(fighter.module_accessor) {
...
pub fn install() {
	Agent::new("zelda")
        .game_acmd("game_specialsstart_phosphora", zelda_game_specialsstart, Default)
        .game_acmd("game_specialairsstart_phosphora", zelda_game_specialsstart, Default)
        .game_acmd("game_specialsend_phosphora", zelda_game_specialsend, Default)
        .game_acmd("game_specialairsend_phosphora", zelda_game_specialsend, Default)
        .status(Main, *FIGHTER_STATUS_KIND_SPECIAL_S, zelda_specials_main)
        .status(Pre, *FIGHTER_STATUS_KIND_SPECIAL_S, zelda_specials_pre)
		.install();
}